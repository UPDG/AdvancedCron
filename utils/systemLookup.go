...
func (u SystemUser) getUIDuint32() uint32 {
	asInt32, _ := strconv.ParseInt(u.Uid, 10, 32)
	return uint32(asInt32)
}

func (u SystemUser) getGIDuint32() uint32 {
	asInt32, _ := strconv.ParseInt(u.Gid, 10, 32)
	return uint32(asInt32)
}
func (u SystemUser) getShell() string {
	if u.Shell == "false" {
		return "/bin/sh"
	}
	return u.Shell
...
func lookupUserId(uid string) (*SystemUser, error) {
...
func lookupUnixUid(uid int) (*SystemUser, error) {
	var pwd C.struct_passwd
	var result *C.struct_passwd
...
func buildUser(pwd *C.struct_passwd) *SystemUser {
	u := &SystemUser{
...
func lookupUnixGid(gid int) (*user.Group, error) {
	var grp C.struct_group
	var result *C.struct_group
	...
	if result == nil {
		return nil, user.UnknownGroupIdError(strconv.Itoa(gid))
	}
	return buildGroup(&grp), nil
}

func buildGroup(grp *C.struct_group) *user.Group {
	g := &user.Group{
