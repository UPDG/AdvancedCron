...
// AddOnceFunc adds a func to the Cron to be run once on the given schedule.
...
func (c *Cron) Schedule(schedule Schedule, cmd Job, names ...string) {
	...
	if !c.running {
		...
		sort.Sort(byTime(c.entries)) // Keep entries sorted by next running time
		return
	}
